...
This document provides a fully reproducible workflow for the generation of analytical figures presented in the manuscript **“Lowland tapirs (*Tapirus terrestris*) as ecosystem engineers: selective sapling breakage and its structural consequences in Yungas Andean forests”**.

The objective of this workflow is to document the complete data processing and visualization steps required to reproduce all **data-driven figures** included in the manuscript (i.e. figures based on statistical analyses and model outputs), excluding photographic material and maps. Statistical models and analytical decisions underlying these figures are implemented in a separate workflow dedicated exclusively to data analysis. Here, models are used only as inputs for visualization, ensuring a clear separation between analytical inference and figure construction.
...
renovales <- tapir %>% 
  filter(conteo_renovales == 1 & conteo_ramas == 1) %>%  # Keep individual saplings only
  mutate(
...
# Dataset for the analysis of shoot number by condition
...
# Add sample sizes (n) for each species:
# First, create one table with percentages and another with total counts.
# Then join them:
...
# Rank–abundance curves for all species
...
# Linear mixed-effects model for breaking diameter
m_diam <- lmer(
  diametro_quiebre_mm ~ bosque + (1 | parcela_id),
  data = renovales %>% filter(quebrado == "si")
)

# Extract p-value for forest effect
p_diam <- anova(m_diam)["bosque", "Pr(>F)"]

# Figure for breaking diameter
fig_diam <- 
  renovales %>%
  filter(quebrado == "si") %>% 
  ...
  annotate(
    ...
    x = 1.5,
    y = 32,
    label = paste0("p = ", format.pval(p_diam, digits = 3, eps = 0.001))
  )

# Linear mixed-effects model for breaking height
m_altura <- lmer(
...
# Extract p-value for forest effect
p_altura <- anova(m_altura)["bosque", "Pr(>F)"]

# Figure for breaking height
fig_altura <- 
  renovales %>%
  filter(quebrado == "si") %>% 
  ...
  annotate(
    ...
    x = 1.5,
    y = 1.6,
    label = paste0("p = ", format.pval(p_altura, digits = 3, eps = 0.001))
  )
...
# ggsave(filename = "Fig.5_new.png", plot = fig_5, device = "png",
#        dpi = 600, height = 5, width = 3)
```
...
# Create all possible predictor combinations
newdata <- expand.grid(
  bosque = levels(df_altura$bosque),
  quebrado = levels(df_altura$quebrado)
)

# Obtain predicted probabilities for each combination
...
ggplot(df_pred, aes(x = altura_indiv, y = probabilidad, fill = quebrado)) +
  ...
  labs(
    # title = "Predicted probability of height class",
    x = "Height class",
    y = "Probability",
    fill = NULL
  ) +
